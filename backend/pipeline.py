...
class PipelineManager:
    ...
    def load_data(self, content: bytes, filename: str):
        ...
        try:
            ...
            # Re-raise to be caught by endpoint
            raise e
    ...
    def preprocess_data(self, strategy: str):
        ...
        # PHASE 3 RULE: Scaling logic MUST NOT include the target column once selected
        feature_cols = [
        ...
        # We keep self.df (original) intact
        # We create self.df_processed for the scaled version
        ...
        # Update X_processed with only features
    ...
    def split_data_step(self, split_ratio: float):
        ...
        # Features: All numeric columns EXCLUDE target_column
        feature_cols = [
        ...
        y = self.df[self.target_col] # Target always from original or consistent source. 
        # Note: self.y was set in select_target, but let's re-extract to be safe if sources aligned.
        # However, processed len must equal original len.
        ...
        self.X = X # Store X if needed later
        self.y = y
